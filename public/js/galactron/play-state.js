...
var PlayState = function(game) {
	...
	this.preload = function(){
		game.load.spritesheet('player_life', 'images/galactron/player_life.png', 13, 10, 1);
...
//		events = new ActionChain();
		...
		this.livesSprites = game.add.group();
...
//		FlxG.bgColor = 0xff000000;
		...
		// add layers to the stage from background to foreground
		...
		// create controls
	 	...
	 	this.controls.fire = game.input.keyboard.addKey(Phaser.Keyboard.SPACEBAR);
	 ...
	 * @param	FlxSprite
	 */
	this.addStatic = function(sprite) {
	...
	this.update = function()	{
		this.playerInput(game.time.elapsed);
	 ...
	 */
	this.playerInput = function(elapsed)	{
		keys = this.controls;
	...
	/**
	 * Factory method which creates the background for a specific level. Must be implemented in child class
	 */
	 ...
	 * @param	wave
	 */
	this.addWave = function(wave, x, y, enemyType, count, delay)	{
		// defaults
	 ...
	 * @param	enemy
	 */
	this.enemyHit = function(bullet, enemy) {
		bullet.kill();
	 ...
	 * @param	enemy
	 */
	this.playerHit = function(player, enemy) {
		this.player.kill();
	 ...
	 * @param	enemy
	 */
	this.powerUp = function(ship, powerup) {
...
//		Tweener.addTween(moreTxt, { x, time.2, transition"linear" } );
	...
	this.createText = function(text, x, y, color, width) {
